...
public class Google {
    ...
    private boolean printToJson(int pagesCount, List<Document> documents) {
        ...
        for (Document doc : documents) {
            titles.addAll(doc.getElementsByClass(TITLE_CLASS).eachText());
            links.addAll(doc.getElementsByClass(LINK_CLASS).eachText());
            descriptions.addAll(doc.getElementsByClass(DESCRIPTION_CLASS).eachText());

            System.out.println();
        }
        String json = GsonUtil.toJson(titles, descriptions, links);
        if ("".equals(json)) {
            return false;
        }
        try {
